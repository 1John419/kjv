...
// Manual edit of /js/SearchEngine.js
// Class name: SearchEngine ->> TestSearchEngine (export also)
// import changed below
// replace all: kjvDb ->> kjvPureDb
// replace all: kjvWords ->> kjvPureWords

import { binIdx } from '/js/data/binIdx.js';
import { kjvIdx } from '/js/data/kjvIdx.js';
import { kjvLists } from '/js/data/kjvLists.js';
// import { kjvDb, kjvWords } from ' /js/Model/DbModel.js';
import { kjvPureDb, kjvPureWords } from '/js/data/kjvPureDb.js';
...
class TestSearchEngine {
  ...
  buildBins(verseIdx) {
    ...
    let bookBin = kjvBin[binIdx.kjvBinIdx.books].find(
      x => x[binIdx.bookBinIdx.bookIdx] === bookIdx
    );
    if (!bookBin) {
      const wordCount = 0;
      const verseCount = 0;
    ...
    let chapterBin = bookBin[binIdx.bookBinIdx.chapters].find(
      (x) => x[binIdx.chapterBinIdx.chapterIdx] === chapterIdx
    );
    if (!chapterBin) {
      const wordCount = 0;
      const verseCount = 0;
  ...
  buildPatterns() {
    ...
    for (const term of this.terms) {
      ...
      const words = kjvPureWords.filter(x => re.test(x));
      if (words.length > 0) {
  ...
  async buildPhraseVerses() {
    ...
    const verseObjs = await kjvPureDb.verses.bulkGet(allVerses);
  ...
  async buildSets() {
    ...
    this.wordObjs = await kjvPureDb.words.bulkGet(unique);
...
export { TestSearchEngine as TestSearchEngine };
